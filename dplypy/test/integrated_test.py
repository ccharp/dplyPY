...
def test_pipeline(capsys):
    ...
    # merge + fill_na + drop
    output_6 = test_df_3 + merge(test_df_4, on="pclass", how="inner") + fill_na(value=0) + drop(columns='sibsp_y')
    expected_6 = test_df_3.pandas_df.merge(test_df_4.pandas_df, on='pclass', how='inner').fillna(value=0).drop(columns='sibsp_y')
    pd.testing.assert_frame_equal(output_6.pandas_df, expected_6)
    
    # one_hot + select + melt
    output_7 = test_df_1 + one_hot(columns=['class']) + select("sex == 'female'") + melt(id_vars='embark_town', value_vars='alive')
    expected_7 = pd.get_dummies(test_df_1.pandas_df, columns=['class']).query("sex == 'female'").melt(id_vars='embark_town', value_vars='alive')
    pd.testing.assert_frame_equal(output_7.pandas_df, expected_7)

    # drop + side_effect + drop
    output_8 = test_df_1 + drop(columns=['parch', 'adult_male']) + side_effect(lambda df: print(df['who'][0])) + write_file('drop.csv')
